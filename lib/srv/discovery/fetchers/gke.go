...
func (a *gkeFetcher) getGKEClusters(ctx context.Context) (types.KubeClusters, error) {
	...
	for _, gkeCluster := range gkeClusters {
		logger := a.Log.WithField("cluster_name", gkeCluster.Name)
		...
		if trace.IsCompareFailed(err) {
			logger.WithError(err).Debugf("Cluster %q did not match the filtering criteria.", gkeCluster.Name)
			continue
		} else if err != nil {
			logger.WithError(err).Warnf("Failed to discover GKE cluster %q.", gkeCluster.Name)
			continue
		}
