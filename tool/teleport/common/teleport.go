...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	discoveryBootstrapCmd.Flag("policy-name", fmt.Sprintf("Name of the Teleport Discovery service policy. Default: %q", discovery.EC2DiscoveryPolicyName)).Default(discovery.EC2DiscoveryPolicyName).StringVar(&configureDiscoveryBootstrapFlags.config.PolicyName)
