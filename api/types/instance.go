...
func (f InstanceFilter) Match(i Instance) bool {
	...
	if fv := f.Version; fv != "" {
		...
		// the api package so that we can do better matching.
		if !strings.HasPrefix(iv, "v") {
			fv = fv[1:]
		}
		if iv != fv {
			return false
...
// InstanceV1 describes the configuration/status of a unique teleport server identity. Each
...
func (i *InstanceV1) AppendControlLog(entries ...InstanceControlLogEntry) {
	...
	sort.Slice(i.Spec.ControlLog, func(a, b int) bool {
		return i.Spec.ControlLog[a].Time.Before(i.Spec.ControlLog[b].Time)
	})
}
